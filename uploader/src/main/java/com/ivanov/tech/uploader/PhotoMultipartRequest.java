...
public class PhotoMultipartRequest extends StringRequest {
    ...
    public PhotoMultipartRequest(Context context, String filePath, Params params,
    ...
    protected void buildMultipartEntity(Context context)
    {
        Log.e("PhotoMultipartRequest", "buildMultipartEntity mImageFile.getName()="+mFilePath);

        try {
            mBuilder.addBinaryBody(mParams.getPartName(), getInputStream(context, mFilePath, mParams), ContentType.create("image"), mFilePath);
            mBuilder.setMode(HttpMultipartMode.BROWSER_COMPATIBLE);
            mBuilder.setLaxMode().setBoundary("xx").setCharset(Charset.forName("UTF-8"));
        }catch(Exception e){
            Log.e(TAG,"buildMultipartEntity Exception e="+e);
        }
    }

    public static final Bitmap getBitmap(Context context, String filePath, Params params){
    ...
    public static final InputStream getInputStream(Context context, String filePath, Params params){


        ByteArrayOutputStream bos = new ByteArrayOutputStream();
        Bitmap bitmap=getBitmap(context,filePath, params);
